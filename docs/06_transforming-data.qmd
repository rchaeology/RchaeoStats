...
the `summarise()` and `group_by` functions from {dplyr}.
...
Select all columns with a `Metal` artefact. Then
calculate the median for the counts grouped
by `Phase`.
...
the plot. It could make more sense to calculate the porportion of
each artefact within each `Phase`.
...
```{r}
write_csv(total_artefact_count, here("data/total_artefact_count.csv"))
```
